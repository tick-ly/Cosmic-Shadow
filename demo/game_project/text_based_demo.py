...
class BattleManager:
    ...
    def use_skill(self, user: CombatUnit, skill: Skill, target: Enemy) -> bool:
        ...
        if not user.can_use_skill(skill):
            self.combat_log.append(f"❌ {user.name}无法使用{skill.name}！")
            return False
    ...
    def _skill_success(self, user: CombatUnit, skill: Skill, target: Enemy) -> bool:
        """技能成功"""
        print(f"✅ {skill.effect.success_description}")
    ...
    def enemy_attack(self, enemy: Enemy, target: CombatUnit):
        ...
        print(f"\n⚔️ {enemy.name}攻击{target.name}！")
        print(f"💥 造成{damage}点伤害！")

        self.combat_log.append(f"⚔️ {enemy.name}对{target.name}造成{damage}点伤害")
    ...
    def show_battle_status(self, hero: CombatUnit, enemy: Enemy):
        ...
        # 英雄状态
        print(f"\n🦸 {hero.name} ({hero.tier.value})")
        health_bar = self._create_bar(hero.current_health, hero.max_health)
        print(f"❤️ 生命值: {hero.current_health}/{hero.max_health} {health_bar}")
        ...
        print(f"😰 疲劳度: {hero.fatigue}/100 {fatigue_bar}")

        # 敌人状态
        print(f"\n👹 {enemy.name}")
        enemy_health_bar = self._create_bar(enemy.current_health, enemy.max_health)
        print(f"❤️ 生命值: {enemy.current_health}/{enemy.max_health} {enemy_health_bar}")
...
def main():
    ...
    while True:
        ...
        for i, skill in enumerate(hero.skills, 1):
            ...
            can_use = "✅" if hero.can_use_skill(skill) else "❌"
        ...
        try:
            ...
            if choice == "-1":
                ...
                print(f"   造成{damage}点伤害！")
                battle.combat_log.append(f"⚔️ {hero.name}基础攻击造成{damage}伤害")
